...
#### createEchoTrigger(pattern, callback, options)
Create a trigger that executes a Lua function when text is displayed locally via `echo()` or `cecho()`. This fires on locally-displayed output from other scripts, without any server communication being involved.

This is useful for scripts that react to output generated by other scripts — for example, a logging script that intercepts all echoed messages, or a script that responds when another script echoes a status update.

**IMPORTANT:** When using regex patterns, escape backslashes in Lua strings! Use `\\d` not `\d`.

```lua
-- React when any script echoes a health update
...
-- Log all echoed messages to a counter
messageCount = 0
createEchoTrigger("", function()
